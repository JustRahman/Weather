...
namespace weather.ModelView
{
    class ViewModel:ViewModelBase
    {
        ...
        public ViewModel()
        {
            ...
            m_notifyIcon.Icon = new System.Drawing.Icon(@"C:\Data\Programming\source\WPF\weather\bin\Debug\TheAppIcon.ico");
            ...
            #region read json
            if (File.Exists(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeColor.json"))
            {
                String JSONtxt = File.ReadAllText(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeColor.json");
                var accounts = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<colorwpf>>(JSONtxt);
                color_creater = accounts.ToList();
            }
            if (File.Exists(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeLanguage.json"))
            {
                String JSONtxt = File.ReadAllText(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeLanguage.json");
            ...
            if (File.Exists(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeLanguage.json"))
            {
                String JSONtxt = File.ReadAllText(@"C:\Data\Programming\source\WPF\weather\bin\Debug\ForDynamiCodeDetails.json");
                var accounts = Newtonsoft.Json.JsonConvert.DeserializeObject<IEnumerable<WPFDetails>>(JSONtxt);
                detailswpf = accounts.ToList();
            }
            if (File.Exists(@"C:\Data\Programming\source\WPF\weather\bin\Debug\test_png.json"))
            {
                String JSONtxt = File.ReadAllText(@"C:\Data\Programming\source\WPF\weather\bin\Debug\test_png.json");
            ...
            try
            {
                ...
                if (LogginParametrs.language == "English")
                {
                    if (model1.Main.Humidity == 0)
                    {
                        ...
                        icon_hum = @"C:\Data\Programming\source\WPF\weather\bin\Debug\clothes\blue.jpg";
                    }
                    else if (model1.Main.Humidity > 0 && model1.Main.Humidity < 20)
                    {
                        quality = "bad";
                        icon_hum = @"C:\Data\Programming\source\WPF\weather\bin\Debug\clothes\red.jpg";
                    }
                    else if (model1.Main.Humidity > 20 && model1.Main.Humidity < 40)
                    {
                        quality = "Not good";
                        icon_hum = @"C:\Data\Programming\source\WPF\weather\bin\Debug\clothes\yellow.jpg";
                    }
                    else if (model1.Main.Humidity > 40 && model1.Main.Humidity < 60)
                    {
                        quality = "Normal";
                        icon_hum = @"C:\Data\Programming\source\WPF\weather\bin\Debug\clothes\green.jpg";
                    }
                ...
                if (turkmen == false)
                {
                    if (model1.Weather[0].Description.Contains(" "))
                    {
                        ...
                        weather_main = weather_main + " " + model1.Weather[0].Description[0].ToString().ToUpper() + model1.Weather[0].Description.Substring(1);
                }
                ...
                if (turkmen == true)
                {
                    for (int i = 0; i < png_weathers_collector.Count; i++)
                    {
                        ...
                        {
                            string[] spliter = png_weathers_collector[i].png_description.Split(' ');
                            foreach (string item in spliter)
                            {
